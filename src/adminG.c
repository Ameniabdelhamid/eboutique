...
#include "consulteClient.h"
#include"consuteClientsA.h"

  GtkWidget * AdminGwindow;
  GtkWidget *fixed;
gchar *sUtf8;
  GtkWidget *btn1;
  GtkWidget *btn2;
...
char welcome1 [20];
  ...
  char prenomUser[20];
   ...
   idus=iduser;
      ...
      AdminGwindow = gtk_window_new(GTK_WINDOW_TOPLEVEL);
  gtk_window_set_title(GTK_WINDOW( AdminGwindow), "CoinMakeup");
gtk_window_set_icon_from_file(GTK_WINDOW ( AdminGwindow), "images/icon.jpg", NULL);
  gtk_window_set_default_size(GTK_WINDOW( AdminGwindow), 1350, 700);
  gtk_window_set_position(GTK_WINDOW( AdminGwindow), GTK_WIN_POS_CENTER);
  fixed = gtk_fixed_new();
  gtk_container_add(GTK_CONTAINER( AdminGwindow), fixed);
  ...
  /*tout les parametre des buttons */
    btn1 = gtk_button_new_with_label("Ajouter Admin Magasin");
  gtk_fixed_put(GTK_FIXED(fixed), btn1, 80,550);
  gtk_widget_set_size_request(btn1, 80, 30);
   btn2 = gtk_button_new_with_label("Consulter Admin Magasin");
  ...
  gtk_widget_set_size_request(btn5, 80, 30);
   g_signal_connect(G_OBJECT(AdminGwindow),"destroy",G_CALLBACK(onDestroyAdminG),NULL);
 g_signal_connect(btn1,"button_press_event",G_CALLBACK (onClickAdminG), 1);
 g_signal_connect (btn2,"button_press_event",G_CALLBACK (onClickAdminG), 2);
  g_signal_connect (btn3,"button_press_event",G_CALLBACK (onClickAdminG), 3);
 g_signal_connect (btn4,"button_press_event",G_CALLBACK (onClickAdminG), 4);
  g_signal_connect (btn5,"button_press_event",G_CALLBACK (onClickAdminG), 5);
 //* couleurs du button*///
colorB.pixel = 0;
	colorB.red=63504;
	colorB.green=95094;
	...
	color.green=65535;
	color.blue=65535;
	gtk_widget_modify_bg(GTK_WIDGET(AdminGwindow), GTK_STATE_NORMAL, &color);
...
gtk_entry_set_max_length (GTK_ENTRY (RechercheText), 50);
 ...
 gtk_widget_show_all(AdminGwindow);
...
static gboolean onClickAdminG( GtkWidget *widget,GdkEventButton *event,gpointer data )
{
 if(data == 1)
    {
        gtk_widget_hide(AdminGwindow);
        InscriptionAdmin(argc,argv);
