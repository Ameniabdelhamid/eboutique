...
void ConsulteClient(int argc, char **argv)
{
 ...
 consCWindow = gtk_window_new(GTK_WINDOW_TOPLEVEL);
  gtk_window_set_title(GTK_WINDOW(consCWindow), "CoinMakeup");
gtk_window_set_icon_from_file(GTK_WINDOW (consCWindow), "images/icon.jpg", NULL);
  gtk_window_set_default_size(GTK_WINDOW(consCWindow), 1350, 700);
  gtk_window_set_position(GTK_WINDOW(consCWindow),GTK_WIN_POS_CENTER);
  //g_signal_connect(G_OBJECT(window),"destroy",G_CALLBACK(onDestroyOffre),NULL);

    scrollbar = gtk_scrolled_window_new(NULL, NULL);
    gtk_container_add(GTK_CONTAINER(consCWindow),scrollbar);
...
modButton = gtk_button_new_with_label("Modifier");
  ...
  gtk_fixed_put(GTK_FIXED(fixed),BackButton,1250,600);
     ...
     g_signal_connect(G_OBJECT(consCWindow),"destroy",G_CALLBACK(onDestroyConC),NULL);
  g_signal_connect (effButton,"button_press_event",G_CALLBACK (onClickConC), 1);
  g_signal_connect (BackButton,"button_press_event",G_CALLBACK (onClickConC), 2);
    g_signal_connect (modButton,"button_press_event",G_CALLBACK (onClickConC), 3);
...
//RGB(246^2,249^2,252^2)
	...
	color.green=65535;
	color.blue=65535;
	gtk_widget_modify_bg(GTK_WIDGET(consCWindow), GTK_STATE_NORMAL, &color);


    gtk_widget_show_all(consCWindow);
