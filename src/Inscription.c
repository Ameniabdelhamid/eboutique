...
void insertion_msg(gpointer data);
void Inscription(int argc,char **argv)
{
...
gtk_window_set_icon_from_file(GTK_WINDOW (InscritWindow), "images/icon.jpg", NULL);
  ...
  gtk_window_set_position(GTK_WINDOW(InscritWindow), GTK_WIN_POS_CENTER);
   g_signal_connect(G_OBJECT(InscritWindow),"destroy",G_CALLBACK(onDestroyInscription),NULL);
fixed = gtk_fixed_new();
...
BackButton = gtk_button_new_with_label("Retour");
  ...
  g_signal_connect (confirmButton,"button_press_event",G_CALLBACK (onClickInscription), 1);
  g_signal_connect (BackButton,"button_press_event",G_CALLBACK (onClickInscription), 2);
    g_signal_connect (resetButton,"button_press_event",G_CALLBACK (onClickInscription), 3);
    /* entry properties */
...
gtk_widget_show(InscritWindow);
    ...
    void onDestroyInscription(GtkWidget *pWindow, gpointer data)
{
    gtk_main_quit();
}
static gboolean onClickInscription( GtkWidget *widget,GdkEventButton *event,gpointer data )
{
 if(data == 1)
    {
