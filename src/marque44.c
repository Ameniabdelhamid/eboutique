...
char welcome1 [20];
  char nomUser[20];
  char prenomUser[20];
void Marque44(int argc, char **argv,char *email,char *iduser,char * nom,char *prenom)
{

/* tout les parametres de mon fenetre signupWindow" */
 Marque44Window= gtk_window_new(GTK_WINDOW_TOPLEVEL);
  gtk_window_set_title(GTK_WINDOW(Marque44Window), "CoinMakeup ");
gtk_window_set_icon_from_file(GTK_WINDOW (Marque44Window), "images/icon.jpg", NULL);
  gtk_window_set_default_size(GTK_WINDOW(Marque44Window), 1350, 700);
  gtk_window_set_position(GTK_WINDOW(Marque44Window), GTK_WIN_POS_CENTER);
   g_signal_connect(G_OBJECT(Marque44Window),"destroy",G_CALLBACK(onDestroyMarque44),NULL);
fixed = gtk_fixed_new();
gtk_container_add(GTK_CONTAINER(Marque44Window),fixed);
GtkWidget * SlidIm= gtk_image_new_from_file("images/logo.jpg");
  ...
  GdkColor color;
	...
	color.green=65535;
	color.blue=65535;
	gtk_widget_modify_bg(GTK_WIDGET(Marque44Window), GTK_STATE_NORMAL, &color);
GtkWidget * resetButton = gtk_button_new_with_label("Retour");

     g_signal_connect (resetButton,"button_press_event",G_CALLBACK (onClickMarques44), 1);
        ...
        if(mysql_real_connect(mysql,"localhost","root","","eboutique",0,NULL,0))
         {
            ...
            mysql_query(mysql,"select  * from produits where marque='nxy'");
...
GtkWidget * transImageS = gtk_image_new_from_file(row[6]);
 ...
 gtk_fixed_put(GTK_FIXED(fixed),transButtonS,x,y+10);
     g_signal_connect (transButtonS,"button_press_event",G_CALLBACK (onClickMarques44), 2);
x=x-30;
//gtk_fixed_put(GTK_FIXED(fixed),produitImage,x,y+10);
    ...
    sprintf(prenomUser,prenom);
	 ...
	 gtk_widget_show_all(Marque44Window);

  gtk_main();
}
    void onDestroyMarque44(GtkWidget *pWindow, gpointer data)
{
    gtk_main_quit();
}
static gboolean onClickMarques44( GtkWidget *widget,GdkEventButton *event,gpointer data )
{
 if(data == 1)
    {

        gtk_widget_hide(Marque44Window);
...
**/
        gtk_widget_hide(Marque44Window);
          Marque44(argc,argv,text,idus,nomUser,prenomUser);
