...
/* tout les parametres de mon fenetre signupWindow" */
 Marque33Window= gtk_window_new(GTK_WINDOW_TOPLEVEL);
  gtk_window_set_title(GTK_WINDOW(Marque33Window), "CoinMakeup ");
gtk_window_set_icon_from_file(GTK_WINDOW (Marque33Window), "images/icon.jpg", NULL);
  gtk_window_set_default_size(GTK_WINDOW(Marque33Window), 1350, 700);
  gtk_window_set_position(GTK_WINDOW(Marque33Window), GTK_WIN_POS_CENTER);
   g_signal_connect(G_OBJECT(Marque33Window),"destroy",G_CALLBACK(onDestroyMarque33),NULL);
fixed = gtk_fixed_new();
gtk_container_add(GTK_CONTAINER(Marque33Window),fixed);
GtkWidget * SlidIm= gtk_image_new_from_file("images/logo.jpg");
  ...
  GdkColor color;
	...
	color.green=65535;
	color.blue=65535;
	gtk_widget_modify_bg(GTK_WIDGET(Marque33Window), GTK_STATE_NORMAL, &color);
GtkWidget * resetButton = gtk_button_new_with_label("Retour");

     g_signal_connect (resetButton,"button_press_event",G_CALLBACK (onClickMarques33), 1);
...
GtkWidget * transImageS = gtk_image_new_from_file(row[6]);
 ...
 gtk_fixed_put(GTK_FIXED(fixed),transButtonS,x,y+10);
     g_signal_connect (transButtonS,"button_press_event",G_CALLBACK (onClickMarques33), 2);
x=x-30;
//gtk_fixed_put(GTK_FIXED(fixed),produitImage,x,y+10);
    ...
    sprintf(prenomUser,prenom);
	 ...
	 gtk_widget_show_all(Marque33Window);
 ...
 if(data == 1)
    {
        ...
        gtk_widget_hide(Marque33Window);
