...
/* tout les parametres de mon fenetre signupWindow" */
 Marque3Window= gtk_window_new(GTK_WINDOW_TOPLEVEL);
  gtk_window_set_title(GTK_WINDOW(Marque3Window), "CoinMakeup ");
gtk_window_set_icon_from_file(GTK_WINDOW (Marque3Window), "images/icon.jpg", NULL);
  gtk_window_set_default_size(GTK_WINDOW(Marque3Window), 1350, 700);
  gtk_window_set_position(GTK_WINDOW(Marque3Window), GTK_WIN_POS_CENTER);
   g_signal_connect(G_OBJECT(Marque3Window),"destroy",G_CALLBACK(onDestroyMarque3),NULL);
fixed = gtk_fixed_new();
gtk_container_add(GTK_CONTAINER(Marque3Window),fixed);
GtkWidget * SlidIm= gtk_image_new_from_file("images/logo.jpg");
  ...
  GdkColor color;
	...
	gtk_widget_modify_bg(GTK_WIDGET(Marque3Window), GTK_STATE_NORMAL, &color);


  GtkWidget * resetButton = gtk_button_new_with_label("retour");
     g_signal_connect (resetButton,"button_press_event",G_CALLBACK (onClickMarques3), 1);
       GdkColor colorB;
  //RGB(246^2,249^2,252^2)
	colorB.pixel = 0;
	colorB.red=63504;
...
GtkWidget  * ComLabel = gtk_label_new("Toujours plus de nouveautés sur CoinMakeup !");
    ...
    g_free(sUtf8);
	 ...
	 gtk_widget_show_all(Marque3Window);
