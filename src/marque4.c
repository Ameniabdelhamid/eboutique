...
void Marque4(int argc,char **argv)
{

/* tout les parametres de mon fenetre signupWindow" */
 Marque4Window= gtk_window_new(GTK_WINDOW_TOPLEVEL);
  gtk_window_set_title(GTK_WINDOW(Marque4Window), "CoinMakeup ");
gtk_window_set_icon_from_file(GTK_WINDOW (Marque4Window), "images/icon.jpg", NULL);
  gtk_window_set_default_size(GTK_WINDOW(Marque4Window), 1350, 700);
  gtk_window_set_position(GTK_WINDOW(Marque4Window), GTK_WIN_POS_CENTER);
   g_signal_connect(G_OBJECT(Marque4Window),"destroy",G_CALLBACK(onDestroyMarque4),NULL);
fixed = gtk_fixed_new();
gtk_container_add(GTK_CONTAINER(Marque4Window),fixed);
GtkWidget * SlidIm= gtk_image_new_from_file("images/logo.jpg");
    gtk_fixed_put(GTK_FIXED(fixed),SlidIm,50,100);

        scrolledwindow = gtk_scrolled_window_new (NULL, NULL);
      gtk_widget_show (scrolledwindow);
      gtk_container_add (GTK_CONTAINER (Marque4Window), scrolledwindow);
  ...
  GdkColor color;
	...
	gtk_widget_modify_bg(GTK_WIDGET(Marque4Window), GTK_STATE_NORMAL, &color);


  GtkWidget * resetButton = gtk_button_new_with_label("retour");
     g_signal_connect (resetButton,"button_press_event",G_CALLBACK (onClickMarques4), 1);
       GdkColor colorB;
  //RGB(246^2,249^2,252^2)
	colorB.pixel = 0;
	colorB.red=63504;
        ...
        if(mysql_real_connect(mysql,"localhost","root","","eboutique",0,NULL,0))
         {
            ...
            mysql_query(mysql,"select * from produits where marque='nina ricci'");
...
gtk_entry_set_max_length (GTK_ENTRY (RechercheText), 50);
 gtk_fixed_put(GTK_FIXED(fixed),RechercheText,150,35);
	 gtk_widget_show_all(Marque4Window);

  gtk_main();
}
    void onDestroyMarque4(GtkWidget *pWindow, gpointer data)
{
    gtk_main_quit();
}
static gboolean onClickMarques4( GtkWidget *widget,GdkEventButton *event,gpointer data )
{
 if(data == 1)
    {
        gtk_widget_hide(Marque4Window);
